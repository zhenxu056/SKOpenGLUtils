...
//  GTVideoLib
//
//  Created by Sunflower on 2019/5/13.
//  Copyright © 2019 gtv. All rights reserved.
//

#import "MGTEDFileManage.h"

@interface MGTEDFileManage()

@property (strong, nonatomic) NSFileManager *manager;

@end

@implementation MGTEDFileManage
...
#pragma mark - 删除文件(夹)
+ (BOOL)removeAllFileItemAtPath:(NSString *)path {
    NSArray *pathArray = [self listFilesInDirectoryAtPath:path deep:NO];
    BOOL isSuccess = YES;
    for (NSString *file in pathArray) {
        NSString *fileName = [NSString stringWithFormat:@"%@/%@",path, file];
        isSuccess = [self removeItemAtPath:fileName];
        if (isSuccess == NO) {
            return NO;
        }
    }
...
+ (NSString *)replaceBundleUUIDPathWithPath:(NSString *)path {
    if (![path containsString:@"Application"]) {
        return path;
    }
    NSString *lastPath = path;
    NSMutableArray *pathArray = [NSMutableArray arrayWithArray:[path componentsSeparatedByString:@"/"]];
    NSArray *mainBundleArray = [[NSBundle mainBundle].resourcePath componentsSeparatedByString:@"/"];
    NSString *tmpData = [NSString stringWithFormat:@"%@",pathArray[4]];
    NSString *tmpMainBundleUUID = mainBundleArray[5];
    if ([tmpMainBundleUUID isEqualToString:@"Application"]) {
        tmpMainBundleUUID = mainBundleArray[6];
    }
    if ([tmpData isEqualToString:@"Application"]) {
        [pathArray replaceObjectAtIndex:5 withObject:tmpMainBundleUUID];
    } else {
        [pathArray replaceObjectAtIndex:6 withObject:tmpMainBundleUUID];
    }
    NSMutableString *lastPathString = [[NSMutableString alloc] init];
    for (NSString *str in pathArray) {
        [lastPathString appendFormat:@"%@/",str];
    }
    lastPath = [lastPathString substringWithRange:NSMakeRange(0, lastPathString.length-1)];
    return lastPath;
}
