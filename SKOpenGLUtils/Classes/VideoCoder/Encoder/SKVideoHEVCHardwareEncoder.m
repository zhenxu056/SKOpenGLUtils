...
#import "SKVideoHEVCHardwareEncoder.h"
#import "SKLogManager.h"

@interface SKVideoHEVCHardwareEncoder ()

@property(nonatomic, strong) SKVideoConfig *config;
...
@implementation SKVideoHEVCHardwareEncoder
...
- (instancetype)initWithConfig:(SKVideoConfig *)config delegate:(id<SKVideoEncoderDelegate>)delegate {
    self = [super init];
    if (self) {
...
- (void)initVideoSession {
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"[VideoToolBox ERROR]: vt compression create error");
        _sessionRef = NULL;
        return;
    }
...
- (void)setIsRealTime:(BOOL)isRealTime {
    ...
    OSStatus status = VTSessionSetProperty(_sessionRef,
                                           ...
                                           boolRef);
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set realTime");
    }
}
...
- (void)setProfileLevel:(CFStringRef)profileLevel {
    ...
    OSStatus status = VTSessionSetProperty(_sessionRef,
                                           ...
                                           profileLevel);
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set profileLevel");
    }
}
...
- (void)setFps:(NSUInteger)fps {
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set fps");
    }
}
...
- (void)setGop:(NSUInteger)gop {
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set gop");
    }
}
...
- (void)setBitRate:(NSUInteger)bitRate {
    ...
    if (status1 != noErr) {
        SKVideoCheckStatus(status1, @"fail to set averageBitRate");
    }
    ...
    if (status2 != noErr) {
        SKVideoCheckStatus(status2, @"fail to set bitRate");
    }
}
...
- (void)setIsEnableBFrame:(BOOL)isEnableBFrame {
    ...
    OSStatus status = VTSessionSetProperty(_sessionRef,
                                           ...
                                           boolRef);
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set has b frame");
    }
}
...
- (void)setEntropyMode:(CFStringRef)entropyMode {
    ...
    OSStatus status = VTSessionSetProperty(_sessionRef,
                                           ...
                                           entropyMode);
    if (status != noErr) {
        SKVideoCheckStatus(status, @"fail to set entropyMode");
    }
}
...
- (void)encodeVideoBuffer:(CMSampleBufferRef)buffer userInfo:(void *)userInfo {
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"encode frame error");
...
void vtHEVCCompressionOutputCallback(void *outputCallbackRefCon,
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"compression to HEVC error");
        return;
    }
    
    SKVideoHEVCHardwareEncoder *encoder = (__bridge SKVideoHEVCHardwareEncoder *)outputCallbackRefCon;
    if (!encoder.delegate) {
    ...
    if (isKeyFrame) {
        do {
            ...
            OSStatus vpsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(formatDescRef,
                                                                                    ...
                                                                                    0);
            if (vpsStatus != noErr) {
                SKVideoCheckStatus(vpsStatus, @"获取 vps error");
                break;
            }
            ...
            OSStatus spsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(formatDescRef,
                                                                                    ...
                                                                                    0);
            if (spsStatus != noErr) {
                SKVideoCheckStatus(spsStatus, @"获取 sps error");
                break;
            }
            ...
            OSStatus ppsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(formatDescRef,
                                                                                    ...
                                                                                    0);
            if (ppsStatus != noErr) {
                SKVideoCheckStatus(ppsStatus, @"获取 pps error");
                break;
            }
    ...
    if (dataStatus != noErr) {
        SKVideoCheckStatus(status, @"获取 已压缩数据 失败");
        return;
    }
