...
#import "SKVideoH264HardwareEncoder.h"
#import "SKLogManager.h"

@interface SKVideoH264HardwareEncoder ()
...
@implementation SKVideoH264HardwareEncoder
...
void vtH264CompressionOutputCallback(void *outputCallbackRefCon,
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"compression to h264 error");
        return;
    }
    
    if (!CMSampleBufferDataIsReady(sampleBuffer)) return; // 数据是否准备好
    
    SKVideoH264HardwareEncoder *encoder = (__bridge SKVideoH264HardwareEncoder *)outputCallbackRefCon;
    if (!encoder.delegate) {
