...
@implementation SKVideoH264HardwareDecoder

@synthesize delegate = _delegate;

- (instancetype)initWithSps:(NSData *)sps pps:(NSData *)pps delegate:(id<SKVideoDecoderDelegate>)delegate {
    self = [super init];
    if (self) {
        self.sps = sps;
        self.pps = pps;
...
- (void)initVideoSession {
    ...
    status = CMVideoFormatDescriptionCreateFromH264ParameterSets(kCFAllocatorDefault,
                                                                 ...
                                                                 &descriptionRef);
    if (status != noErr) {
        SKVideoCheckStatus(status, @"[VideoToolBox ERROR]: vt paramterSets create error");
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"[VideoToolBox ERROR]: vt decompression create error");
...
- (void)decodeVideoData:(NSData *)data pts:(int64_t)pts dts:(int64_t)dts fps:(int)fps userInfo:(void *)userInfo {
    ...
    do {
        ...
        if (statuts != kCMBlockBufferNoErr || !blockBufRef) {
            SKVideoCheckStatus(statuts, @"create block buffer error");
            break;
        }
        ...
        if (statuts != kCMBlockBufferNoErr || !sampleBufRef) {
            SKVideoCheckStatus(statuts, @"create sample buffer error");
            break;
        }
        ...
        if (statuts == kVTInvalidSessionErr) {
            SKVideoCheckStatus(statuts, @"decompress error");
            [self uninitVideoSession];
            [self initVideoSession];
            break;
        }
...
void vtH264DecompressionOutputCallback(void * CM_NULLABLE decompressionOutputRefCon,
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"decompression to h264 error");
        return;
    }
    
    
    
    SKVideoH264HardwareDecoder *decoder = (__bridge SKVideoH264HardwareDecoder *)decompressionOutputRefCon;
    if (!decoder.delegate) {
    ...
    do {
        ...
        if (status1 != noErr) {
            SKVideoCheckStatus(status1, @"create description buffer error");
            break;
        }
        ...
        status1 = CMSampleBufferCreateForImageBuffer(kCFAllocatorDefault,
                                                    ...
                                                    &sampleBufRef);
        if (status1 != noErr) {
            SKVideoCheckStatus(status1, @"create sample buffer error");
            break;
        }
