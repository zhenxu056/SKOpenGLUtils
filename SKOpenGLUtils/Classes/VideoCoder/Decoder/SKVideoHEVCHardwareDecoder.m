...
#import "SKVideoHEVCHardwareDecoder.h"
#import "SKLogManager.h"

@interface SKVideoHEVCHardwareDecoder () {
...
@implementation SKVideoHEVCHardwareDecoder
...
void vtHEVCDecompressionOutputCallback(void * CM_NULLABLE decompressionOutputRefCon,
    ...
    if (status != noErr) {
        SKVideoCheckStatus(status, @"decompression to hevc error");
        return;
    }
    
    
    
    SKVideoHEVCHardwareDecoder *decoder = (__bridge SKVideoHEVCHardwareDecoder *)decompressionOutputRefCon;
    if (!decoder.delegate) {
