...
@property(nonatomic, strong, readwrite) SKAudioIOUnit *IO;
@property(nonatomic, strong, readwrite) SKAudioMixUnit *Mix;

@end


@implementation SKAudioUnit
...
- (instancetype)initWithComponentDesc:(AudioComponentDescription)componentDesc
    ...
    if (self) {
        ...
        if (nil != error) *error = status != noErr ? [SKAudioError errorWithType:DVAudioError_notErr] : nil;
...
- (SKAudioMixUnit *)Mix {
    if (_Mix == nil) {
        _Mix = [[SKAudioMixUnit alloc] init];
...
#pragma mark - <-- Method -->
- (BOOL)setupUnitConfig:(void (^)(SKAudioUnit * _Nonnull))block {
