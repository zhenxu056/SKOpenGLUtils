...
- (NSMutableArray<SKAudioPacket *> *)packetBuffer {
    if (!_packetBuffer) {
...
- (void)playAudioData:(uint8_t *)data size:(UInt32)size {
    ...
    SKAudioPacket *packet = [[SKAudioPacket alloc] initWithData:data size:size];
...
OSStatus recordCallback(void *inRefCon,
    ...
    SKAudioIOUnit *object = (__bridge SKAudioIOUnit*)inRefCon;
    ...
    [object.wAudioUnit.delegate DVAudioUnit:object.wAudioUnit
                                 recordData:data
                                      error:status == noErr ? nil : [SKAudioError errorWithType:DVAudioError_notRecord]];
...
OSStatus playCallback(void *inRefCon,
    ...
    if (object.packetBuffer.count > 0) {
        ...
        SKAudioPacket *packet = object.packetBuffer.firstObject;
