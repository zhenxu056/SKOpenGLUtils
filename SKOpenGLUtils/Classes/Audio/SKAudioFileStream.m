...
{
    SKAudioFileStream *audioFileStream = (__bridge SKAudioFileStream *)inClientData;
...
@implementation SKAudioFileStream
...
- (void)handleAudioFileStreamPackets:(const void *)packets numberOfBytes:(UInt32)numberOfBytes numberOfPackets:(UInt32)numberOfPackets packetDescription:(AudioStreamPacketDescription *)packetDescriptions
{
    ...
    for (int i = 0; i < numberOfPackets; i++) {
        ...
        SKParseAudioData *parsedData = [SKParseAudioData parsedAudioDataWithBytes:packets+packetOffset packetDescription:packetDescriptions[i]];
